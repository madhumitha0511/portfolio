    ...
    <section id="experience" className="py-20 px-4 relative overflow-hidden bg-[color:var(--color-bg)]">
      <div className="max-w-full mx-auto relative z-10 px-6">
        ...
        {experiences.length === 0 ? (
          <p className="text-center text-[color:var(--color-muted)]">No experiences yet.</p>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 relative">
              <motion.div
                ...
                className="rounded-3xl overflow-hidden border border-[color:var(--color-border)] relative shadow-elevated"
                style={{
                  background: "linear-gradient(135deg, var(--color-bg-elevated) 0%, var(--color-card) 100%)",
              ...
              >
                ...
                {/* Year Filter Tabs */}
                <div className="px-6 py-3 bg-[color:var(--color-bg)]/80 border-b border-[color:var(--color-border)]/50 flex gap-2 overflow-x-auto scrollbar-hide relative z-10">
                  <motion.button
                        ...
                        return (
                          <motion.button
                            ...
                            className={`relative rounded-2xl p-5 border-2 transition-all overflow-hidden group backdrop-blur-sm h-full text-[color:var(--color-text)] ${
                              isActive
                                ? "bg-[color:var(--color-card)] border-[color:var(--color-primary)] shadow-soft ring-2 ring-[color:var(--color-primary)]/50"
                                : "bg-[color:var(--color-bg-elevated)]/70 border-[color:var(--color-border)]/60 hover:border-[color:var(--color-primary)]/70 hover:bg-[color:var(--color-bg-elevated)]"
                            }`}
                            style={{ cursor: cursorUrl }}
                          >
                            {isActive && (
                              <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="absolute inset-0 bg-[color:var(--color-primary)]/10 pointer-events-none"
                              />
                            )}
                            ...
                            <div className="relative flex flex-col h-full">
                              ...
                              <div className="border-t border-[color:var(--color-border)]/60 my-2" />
              ...
              <AnimatePresence mode="wait">
                {activeExp && (
                  <motion.div
                    ...
                    className="w-full rounded-2xl border border-[color:var(--color-border)] backdrop-blur-xl p-6 md:p-7 shadow-elevated"
                    style={{
                      background: "linear-gradient(135deg, var(--color-bg-elevated) 0%, var(--color-card) 100%)",
                    }}
                  >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>
                            {activeExp.start_date
                              ? new Date(activeExp.start_date).toLocaleDateString("en-US", {
                                  month: "short",
                                  year: "numeric",
                                })
                              : ""}{" "}
                            ...
                            {activeExp.is_current
                              ? "Present"
                              : activeExp.end_date
                              ? new Date(activeExp.end_date).toLocaleDateString("en-US", {
                                  month: "short",
                                  year: "numeric",
                                })
                              : ""}
                    ...
                    <div className="border-t border-[color:var(--color-border)] my-4" />
